 ...
 */
class PlayoffForm extends FormBase {
   /**
   *
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['descptn_1'] = [
      '#type' => 'item',
      '#title' => $this->t('Select the Playoff team then select next to submit into the week'),
    ];
              
    $unm = $this->current_user->getDisplayName();
    $query = $this->entity_query->get('node')
        ->condition('title', $unm, 'CONTAINS')
        ->condition('type', 'playoff');
        $nids = $query->execute();
 ...
 $header = [
    'life_name' => $this->t('Choose Life'),
        'wk_1' => $this->t('Wild Card'),
        'wk_2' => $this->t('Divisional'),
        'wk_3' => $this->t('Confrance'),
        'wk_4' => $this->t('Superbowl')
  ];
